...
/* cron job for updating the dhl tracking status */
/* created 2019 by jb */
...
/* TODO - check if the order is already shipped and work only with allowed order_states */
$disable_order_state = array(0, 5, 6, 7, 20, 21, 24, 26, 28, 31, 41, 47);
...
while ($i < 1 && count($_get_orders) > 0) {
    ...
    $i = 1;
    // DHL pakete holen, können auch mehrere sein
    ...
    if (count($_get_dhl_label) > 0) {
        foreach ($_get_dhl_label as $mylabel) {
            ...
            echo $shipping_number . " -> " . $id_sxdhl . " -> " . $state . "\n";

            if ($state != "DLVRD") {
                $tracking_result = $track->getState($shipping_number);
                echo $shipping_number . " will be checked....\n";
        ...
        $_sql_get_shipping_from_orders = "select shipping_number from ps_orders "
            . " where id_order = '$id_order'";
